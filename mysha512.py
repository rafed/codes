...
def sha512Process(chunk):
    ...
    buf[0] = (buf[0] + a) & 0xffffffffffffffff
    buf[1] = (buf[1] + b) & 0xffffffffffffffff
    buf[2] = (buf[2] + c) & 0xffffffffffffffff
    buf[3] = (buf[3] + d) & 0xffffffffffffffff
    buf[4] = (buf[4] + e) & 0xffffffffffffffff
    buf[5] = (buf[5] + f) & 0xffffffffffffffff
    buf[6] = (buf[6] + g) & 0xffffffffffffffff
    buf[7] = (buf[7] + h) & 0xffffffffffffffff
...
def sha512(message):
    ...
    for i in range(len(message) // 1024):
        sha512Process(message[i * 1024:i * 1024 + 1024])

    return ''.join(hex(i)[2:] for i in buf)
